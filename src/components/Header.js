...
class Header extends React.Component {
  ...
  render() {
    ...
    const menuStyle = {display: (this.state.isMenuDisplayed ? 'flex' : 'none')};
    ...
    return (
      <Container>
        ...
        <HeaderBox>
          <ButtonIcon
            src={menuImg}
            onClick={()=>(this.setState((oldState)=>({isMenuDisplayed: !oldState.isMenuDisplayed})))}
          />
          <MenuContainer>
            <HeaderMenu
              style={menuStyle}
              collapse={()=>(this.setState((oldState)=>({isMenuDisplayed: !oldState.isMenuDisplayed})))}
            />
          </MenuContainer>
          <Switch>
            <Route path='/chats' />
...
function ChatTitle(props) {
  ...
  const {displayName, icon} = data.chats[chatName];
